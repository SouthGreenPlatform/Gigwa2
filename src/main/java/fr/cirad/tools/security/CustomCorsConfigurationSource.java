...
public class CustomCorsConfigurationSource extends UrlBasedCorsConfigurationSource {
    ...
    public static final String CORS_PROPERTY_PREFIX = "corsOrigins_";
        ...
        public CustomCorsConfigurationSource(AppConfig appConfig) {
            ...
            Map<String, String> corsConfigEntries = appConfig.getPrefixed(CORS_PROPERTY_PREFIX);
            for (String key : corsConfigEntries.keySet())
                if (key.startsWith(CORS_PROPERTY_PREFIX))
                    customConfigs.put(key.substring(CORS_PROPERTY_PREFIX.length()).replaceFirst(".*/rest/", "/"), Arrays.stream(corsConfigEntries.get(key).split(",")).map(String :: trim).collect(Collectors.toList()));
